classdef e0509 < RobotBaseClass
    %% LinearUR5 UR5 on a non-standard linear rail created by a student

    properties(Access = public)              
        plyFileNameStem = 'e0509';
    end
    
    methods
%% Define robot Function 
        function self = e0509(baseTr)
			self.CreateModel();
            if nargin < 1			
				baseTr = eye(4);				
            end
...
%% Create the robot model
        function CreateModel(self)   
            ...
            link(3) = Link('d',0,'a',-0.24355,'alpha',0,'qlim', deg2rad([-90 90]), 'offset',0);%-360 360
            link(4) = Link('d',0,'a',-0.2132,'alpha',0,'qlim', deg2rad([-130 130]), 'offset', 0);
